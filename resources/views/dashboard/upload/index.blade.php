...
@section('body')
    <section class="card col-span-2 xl:col-span-1 p-3">
        ...
        <table class="table-auto w-full text-right">
            <thead>
            <tr class="border-t">
                <th class="px-4 py-2 border-r">لیست آپلود فایل</th>
                <td class="px-4 py-2 border-l" colspan="6">
                    <a href="{{route('upload.create' , ['page' => $items->currentPage()])}}"
            ...
            <tr>
                ...
                <th class="px-4 py-2 border-r border-t">آدرس</th>
                ...
                @foreach($items as $value)
                    <tr>
                        ...
                        <td class="border border-l-0 px-4 py-2">
                            @if($value->photo)
                                <a target="_blank" href="{{url($value->photo)}}"> مشاهده </a>
                            @endif
                        </td>
                        <td class="border border-l-0 px-4 py-2">
                            <a href="{{route('upload.edit' , [$value->id , 'page' => $items->currentPage()])}}"
                        ...
                        <td class="border px-4 py-2 border-l">
                            <form class="delete-form text-center" method="post"
                                  action="{{route('upload.destroy' , $value->id)}}">
                                @method('delete')
                                @csrf
