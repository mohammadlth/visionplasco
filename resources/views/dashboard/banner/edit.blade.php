...
@section('body')
    ...
    <section class="card col-span-2 xl:col-span-1 p-3">
        ...
        <div class="mt-3">
            <form method="post" action="{{route('banners.update' , $item->id)}}" enctype="multipart/form-data">
                @method('patch')
                @csrf
                ...
                <div class="grid grid-cols-2 gap-5">
                    ...
                    <div class="input-group">
                        <label for="view_in" class="text-sm text-gray-600">دسته</label>
                        <select name="view_in"
                                class="border rounded-sm px-2 py-1 bg-gray-100 select2-category select_view_in">

                            <option {{$item->view_in == 'home' ? ' selected ' : null  }}  value="home">
                                خانه
                            </option>

                            <option {{$item->view_in == 'product' ? ' selected ' : null  }}  value="product">
                                صفحه تک محصول
                            </option>

                            <option {{$item->view_in == 'slider' ? ' selected ' : null  }}  value="slider">
                                تصویر ساب صفحه اصلی
                            </option>

                            <option {{$item->view_in == 'buyer' ? ' selected ' : null  }}  value="buyer">
                                صفحه لیست خریداران (پاپاپ)
                            </option>
                            <option {{$item->view_in == 'category' ? ' selected ' : null  }}  value="category">
                                دسته بندی محصولات
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="input-group select-category"
                         style="display: {{$item->view_in == 'category'  ? ' grid ' : ' none '}}">
                        <label for="sort" class="text-sm text-gray-600">دسته</label>
                        <select name="page_category_id"
                                class="border rounded-sm px-2 py-1 bg-gray-100 select2-category">
                            <option value="">انتخاب کنید</option>
                            @foreach($categories as $value)
                                @include('components.dashboard.category.select' , ['item' => $value  ,  'selected' => $item->page_category_id])
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="link" class="border rounded-sm px-2 py-1 bg-gray-100"
                               value="{{$item->link}}" placeholder="لینک به">
                    </div>
                    ...
                    <div class="input-group">
                        ...
                        <textarea rows="1" name="short_text" class="border rounded-sm px-2 py-1 bg-gray-100"
                                  placeholder="توضیح کوتاه">{{$item->short_text}}</textarea>
                    </div>
