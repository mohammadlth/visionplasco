...
@section('body')
    <section class="card col-span-2 xl:col-span-1 p-3">
        ...
        <table class="table-auto w-full text-right">
            <thead>
            <tr class="border-t">
                <th class="px-4 py-2 border-r">لیست بنر ها</th>
                <td class="px-4 py-2 border-l" colspan="6">
                    <a href="{{route('banners.create' , ['page' => $items->currentPage()])}}"
            ...
            @if(count($items) > 0)
                @foreach($items as $value)
                    <tr>
                        ...
                        <td class="border border-l-0 px-4 py-2">
                            <a href="{{route('banners.edit' , [$value->id , 'page' => $items->currentPage()])}}"
                        ...
                        <td class="border px-4 py-2 border-l">
                            <form class="delete-form text-center" method="post"
                                  action="{{route('banners.destroy' , $value->id)}}">
                                @method('delete')
                                @csrf
